...
# ============================================================
# Project Infinity X Garnet Build Script
# ============================================================
...
description() {
    ...
    echo -e "${CYAN}${BOLD}├────────────────────────────────────────────────────────────┤${RESET}"
    echo -e "${CYAN}│${RESET} Project Infinity X automated build and release script for       ${CYAN}│${RESET}"
...
notify_build_start() {
    ...
    tg_send "🚀 Project Infinity X build started
...
notify_build_failed() {
    ...
    tg_send "❌ Project Infinity X build FAILED
...
notify_artifact() {
    ...
    local MESSAGE="📦 Project Infinity X
...
notify_finished() {
    ...
    tg_send "🏁 Project Infinity X build finished
...
ok "Project Infinity X repository initialized"
...
section "Building Project Infinity X"
...
if [[ "${BUILD_SUCCESS}" != "1" ]]; then
    ...
    fail "Project Infinity X build failed"
...
ok "Project Infinity X build successful"
info "Build time: ${BUILD_MINUTES} minutes"
...
info "Searching for the final Project Infinity X ZIP (target_files and OTA helper ZIPs excluded)"
...
if [[ ${#ARTIFACTS[@]} -eq 0 ]]; then
    ...
    tg_send "⚠️ Project Infinity X build completed
...
echo -e "${GREEN}${BOLD}"
echo "╔════════════════════════════════════════════════════════════╗"
echo "║                  Project Infinity X COMPLETE                      ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo -e "${RESET}"
